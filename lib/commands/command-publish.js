...
module.exports = {
    ...
    options: {
        ...
        'skip-pre-release-checks': {
            describe: 'Skip checks to ensure no pre-release versions',
            type: "boolean",
            default: false
        }
    },
    ...
    validate: function(args, rapido) {
        return {
            ...
            skipPreReleaseChecks: args['skip-pre-release-checks'] === true
        };
    },
    ...
    run: function(args, config, rapido) {
        ...
        function ensureNoPreReleaseVersions(recursive) {
            // if (packageManifest.parsedVersion.label || args.skipPreReleaseChecks) {
        ...
        function updatePatchVersion() {
            ...
            function checkPublished() {
                ...
                return rapido.util.request({
                        url: url.resolve(npmRegistryRead, '/' + packageManifest.name + '/' + currentVersion),
                        logResponses: false,
                        logger: logger
                    })
                    .then(function(data) {
                ...
                .then(function(publishedCheckResult) {
                    if (publishedCheckResult.published) {
                        ...
                        if (packageManifest.parsedVersion.label) {
                            packageManifest.parsedVersion.incBuild();
                        } else {
        ...
        function npmPublish() {
            ...
            var label = packageManifest.parsedVersion.label ? 'latest-' + packageManifest.parsedVersion.label : 'latest';

            var npmPublishArgs = ['publish', '--tag', label, '--registry', npmRegistryWrite];
